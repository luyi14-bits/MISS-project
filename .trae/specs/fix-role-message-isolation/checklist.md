...
## Bug A: 角色切换消息隔离
- [ ] `OnCurrentRoleChanged` 中有 `LocalStore.SaveMessages` 保存当前消息
- [ ] `OnCurrentRoleChanged` 中有 `_allMessages.Clear()` 清空
- [ ] `OnCurrentRoleChanged` 中有按 `RoleName` 过滤的 `LocalStore.LoadMessages` 重新加载
- [ ] `Filter` 的 `IsUser` 规则不变（L62-L68 无需修改）
