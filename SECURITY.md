...
> **Daniel Miessler**：安全不是一次性工程，是持续纪律。这份 Policy 是纪律的起点。  
> **James Kettle**：如果你找到了没在这里列出的漏洞，请报告。我们不会让它留在已发布的版本里。  
> **Tavis Ormandy**：你的代码跑在用户机器上。build.ps1 确保我们不会把 .pdb 和 .db 打包进去。基础工程做对了。
